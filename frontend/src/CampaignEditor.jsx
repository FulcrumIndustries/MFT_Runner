...
export default function CampaignEditor({ open, campaign, onClose, onSave }) {
  ...
  const addFilePolicy = () => {
    setFormState({
      ...formState,
      FilesizePolicies: [
        ...formState.FilesizePolicies,
        { Size: 1, Unit: "KB", Percent: 0 },
      ],
    });
  };

  const removeFilePolicy = (index) => {
    const newPolicies = formState.FilesizePolicies.filter(
      (_, i) => i !== index
    );
    setFormState({ ...formState, FilesizePolicies: newPolicies });
  };
  ...
  const validateCampaign = () => {
    ...
    if (!formState.Hostname || !formState.Port || !formState.Path) {
      alert("Please fill all required fields (Hostname, Port, Path)");
      return false;
    }

    // Validate protocol selection
    if (!["FTP", "SFTP", "HTTP", "HTTPS"].includes(formState.Protocol)) {
      alert("Invalid protocol selected");
      return false;
    }

    // Validate file size percentages
    const totalPercent = formState.FilesizePolicies.reduce(
      (sum, policy) => sum + policy.Percent,
      0
    );
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
          ...
          >
            ...
            <FormControl sx={{ width: 120 }}>
              ...
              <Select
                value={policy.Unit}
                onChange={(e) =>
                  handleFilePolicyChange(index, "Unit", e.target.value)
                }
                label="Unit"
              >
      ...
      <DialogActions>
        ...
        <Button
          onClick={() => validateCampaign() && onSave(formState)}
          variant="contained"
          disabled={
            formState.FilesizePolicies.reduce(
              (sum, policy) => sum + policy.Percent,
              0
            ) !== 100 ||
            !formState.Hostname ||
